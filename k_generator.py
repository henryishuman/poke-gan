...
class GAN():
    def __init__(self):
        ...
        self.img_rows = output_size 
        self.img_cols = output_size
        self.channels = 3
    ...
    def train(self, input_path, output_path, epochs, batch_size=128, save_interval=50):

        # Input the images from the directory
        X_train = self.get_batch(glob(os.path.join(input_path, '*.jpg'))[:5000], output_size, output_size, 'RGB')
        ...
        for epoch in range(epochs):
            ...
            # Print the progress
            print ("%.2f%% %d [D loss: %f, acc.: %.2f%%] [G loss: %f]" % (epoch/epochs*100, epoch, d_loss[0], 100*d_loss[1], g_loss))
            ...
            if epoch % save_interval == 0:
                self.save_imgs(output_path, epoch)
        ...
        # At the end, draw final model images
        self.save_imgs(output_path, 'final')

        # Save the models for later use
        self.discriminator.save('./models/discriminator')
        self.generator.save('./models/generator')
    ...
    def save_imgs(self, output_path, filename):
        r, c = 5, 5
    ...
    def load_model_from_file(self, generator_filepath, discriminator_filepath):
...
if __name__ == '__main__':
    ...
    epochs = 10000
    total_images = 100
    save_interval = epochs // total_images
    gan.train('./images/input', './images/output', epochs=epochs, batch_size=16, save_interval=save_interval)
